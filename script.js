...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (skillBoxes && skillModal) {
    skillBoxes.forEach((box) => {
      box.addEventListener("click", () => {
        const skillId = box.getAttribute("data-skill");

        // Check if skillsData is available
        if (typeof skillsData !== "undefined" && skillsData[skillId]) {
          ...
          document.getElementById("skill-what").textContent = skill.what;
          document.getElementById("skill-learn").textContent = skill.learn;
          ...
          document.getElementById("skill-demand").textContent = skill.demand;
          document.getElementById("skill-companies").textContent =
            skill.companies;
          document.getElementById("skill-career").textContent = skill.career;
          ...
          alert("Skill information not available. Please try again later.");
        }
      });
    });
  ...
  if (careerBoxes && careerModal) {
    careerBoxes.forEach((box) => {
      box.addEventListener("click", () => {
        const careerId = box.getAttribute("data-career");

        // Check if careersData is available
        if (typeof careersData !== "undefined" && careersData[careerId]) {
          const career = careersData[careerId];

          // Populate modal with career data
          ...
          console.error("Career data not found for:", careerId);
          alert("Career information not available. Please try again later.");
        }
      });
    });
    ...
    // Close career modal
    if (closeCareerModal) {
      closeCareerModal.addEventListener("click", () => {
        // Animate modal out
